...
> 来源：https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html
> 此文件为正文转换快照；外链及远程图片需要联网，公式和排版以原始资料为准。

# Fundamental Concepts[](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#fundamental-concepts "Permalink to this heading")
...
## Images[](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#images "Permalink to this heading")
...
is visually illustrated in [this figure](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#lbl-image-metadata).

![Image meta-data.](https://simpleitk.readthedocs.io/en/master/_images/ImageOriginAndSpacing.svg)
...
a voxel beyond the last voxel.[](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#id5 "Permalink to this image")
...
that [below](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#lbl-metric-units).
...
[this figure](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#lbl-isotropy).

![nonisotropic vs. isotropic pixels.](https://simpleitk.readthedocs.io/en/master/_images/nonisotropicVsIsotropic.svg)

The same image displayed with a viewer that is not aware of spatial meta-data
(left image) and one that is aware (right image). The image’s pixel spacing is (0.97656, 2.0)mm.[](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#id6 "Permalink to this image")
...
acquired at different sites. [This figure](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#lbl-spatial-location)
...
![images are spatial objects](https://simpleitk.readthedocs.io/en/master/_images/spatialRelationship.svg)
...
major order, of (0.7, -0.7, 0.7, 0.7), \(\left[\begin{array}{cc}0.7&-0.7\\0.7&0.7\end{array}\right]\).
The image on the right’s origin is
(16.9, 21.4) with a direction cosine matrix of (1,0,0,1), \(\left[\begin{array}{cc}1&0\\0&1\end{array}\right]\).[](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#id7 "Permalink to this image")
...
units has [not ended well in the past](https://en.wikipedia.org/wiki/Mars_Climate_Orbiter).
...
1. How do you access the pixel values in an image?
   ...
   1. In dynamically typed languages such as Python and R use a type agnostic function:

      ```
      image.GetPixel((0,0))
      ```
   2. In statically typed languages such as C# and C++ use type specific functions:
...
### Channels[](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#channels "Permalink to this heading")
...
[RGB color space](https://en.wikipedia.org/wiki/RGB_color_space). In some cases you
can have the same image type, but the channel values represent another color space, such as [HSV](https://en.wikipedia.org/wiki/HSL_and_HSV) (it decouples the color and intensity
...
### Additional Resources[](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#additional-resources "Permalink to this heading")

1. The API for the SimpleITK
   [Image class](https://simpleitk.org/doxygen/latest/html/classitk_1_1simple_1_1Image.html)
   in Doxygen format.
2. To really understand the structure of SimpleITK images and how to work with them,
   ...
   [SimpleITK Jupyter notebooks](https://github.com/InsightSoftwareConsortium/SimpleITK-Notebooks)
   (Python and R only).

## Transforms[](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#transforms "Permalink to this heading")
...
one after the other \(T\_0(T\_1(T\_2(...T\_n(p)...)))\). The semantics are
...
that is optimized, only the parameters of the last transformation \(T\_n\) will
be optimized over.

### Additional Resources[](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#id1 "Permalink to this heading")
...
1. The API for the SimpleITK transformation classes is available in Doxygen format:
   ...
   * [Euler2DTransform](https://simpleitk.org/doxygen/latest/html/classitk_1_1simple_1_1Euler2DTransform.html)
     and [Euler3DTransform](https://simpleitk.org/doxygen/latest/html/classitk_1_1simple_1_1Euler3DTransform.html).
   * [Similarity2DTransform](https://simpleitk.org/doxygen/latest/html/classitk_1_1simple_1_1Similarity2DTransform.html)
     and [Similarity3DTransform](https://simpleitk.org/doxygen/latest/html/classitk_1_1simple_1_1Similarity3DTransform.html).
...
## Resampling[](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#resampling "Permalink to this heading")
...
2. Resampling grid - a regular grid of points given in coordinate system \(f\)
   which will be mapped to coordinate system \(m\).
3. Transformation \(T\_f^m\) - maps points from coordinate system \(f\)
   to coordinate system \(m\), \(^mp = T\_f^m(^fp)\).
4. Interpolator - method for obtaining the intensity values at arbitrary points
   in coordinate system \(m\) from the values of the points defined by the Image.
...
### Common Errors[](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#common-errors "Permalink to this heading")
...
2. Using the inverse of the transformation \(T\_f^m\). This is a relatively
   common error, which is readily addressed by invoking the transformation’s
   GetInverse method.

### Additional Resources[](https://simpleitk.readthedocs.io/en/master/fundamentalConcepts.html#id3 "Permalink to this heading")

1. The API for the SimpleITK
   [ResampleImageFilter class](https://simpleitk.org/doxygen/latest/html/classitk_1_1simple_1_1ResampleImageFilter.html)
